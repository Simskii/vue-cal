...
export default {
  ...
  methods: {
      ...
      switch (view) {
        ...
        case "month":
          ...
          if (startDate.getDay() !== (this.startWeekOnSunday ? 0 : 1)) {
            startDate = getPreviousFirstDayOfWeek(
              startDate,
          ...
          for (let i = 0; i < 42; i++) {
            ...
            if (currentMonth !== cellDate.getMonth()) {
              ...
              if (dayEvents.length) {
                this.view.outOfScopeEvents.push(
    ...
    onMouseUp(e) {
      ...
      if (
        resizeAnEvent._eid &&
        resizeAnEvent.newHeight !== resizeAnEvent.originalHeight
      ) {
        let event = this.mutableEvents[resizeAnEvent.startDate].find(
          item => item._eid === resizeAnEvent._eid
        );
        if (event) {
        ...
        if (multipleDays) {
          ...
          for (let i = 1; i <= datesDiff; i++) {
            ...
            const linked = [
              ...
              ...eventPieces
                .slice(0)
                .filter(e => e._eid !== eventPieces[i - 1]._eid)
            ];
    ...
    createEvent(formattedDate, e = null, eventOptions = {}) {
      ...
      if (typeof e === "number") startTimeMinutes = e;
        ...
        startTimeMinutes =
          (mouseY * this.timeStep) / parseInt(this.timeCellHeight) +
          this.timeFrom;
      }
  ...
  computed: {
    ...
    minTimestamp() {
      ...
      if (this.minDate && typeof this.minDate === "string")
        date = stringToDate(this.minDate);
    ...
    maxTimestamp() {
      ...
      if (this.maxDate && typeof this.maxDate === "string")
        date = stringToDate(this.maxDate);
    ...
    weekDaysShort() {
      ...
      if (weekDaysShort) {
        ...
        if (this.hideWeekends) {
          weekDaysShort = this.startWeekOnSunday
            ? weekDaysShort.slice(1, 6)
            : weekDaysShort.slice(0, 5);
        }
      }
          ...
          cells = Array.apply(null, Array(12)).map((cell, i) => {
            ...
            return {
              ...
              content: this.xsmall
                ...
                : this.months[i].label,
              current:
                i === this.now.getMonth() && fromYear === this.now.getFullYear()
            };
          });
          break;
        case "month":
          ...
          cells = Array.apply(null, Array(42)).map((cell, i) => {
            ...
            const isToday =
              !todayFound &&
              startDate &&
              startDate.getDate() === this.now.getDate() &&
          ...
          if (this.hideWeekends) {
            cells = cells.filter(
              cell => cell.startDate.getDay() > 0 && cell.startDate.getDay() < 6
            );
          }
          break;
        case "week":
